...
- [x] Open [`01_ingest_traffic.R`](01_ingest_traffic.R) or [`01_ingest_traffic.py`](01_ingest_traffic.py).
- [x] Confirm `METRO_ID` is set to Brussels (default `948`).
- [x] Run the script from inside `12_end/`.
- [x] Confirm `12_end/data/traffic.db` has rows in `traffic` for your `metro_id`.
...
- [x] Open [`.github/workflows/12-ingest-r.yml`](../.github/workflows/12-ingest-r.yml) and [`.github/workflows/12-ingest-python.yml`](../.github/workflows/12-ingest-python.yml) and identify `schedule`, `working-directory`, and commit-back steps.
- [x] Confirm both `ingest_r` and `ingest_python` jobs run, so both language versions stay healthy.
...
- [x] In Python jobs, use `actions/setup-python` with `cache: 'pip'`.
- [x] In R jobs, use `r-lib/actions/setup-r-dependencies` to cache installed packages across runs.
